...
This interoperability standard defines DGWIG implementable provisions that profile the OGC GeoPackage standard to ensure that DGIWG GeoPackages, GeoPackage SQLite Extensions, and supporting utilities and services fulfil their intended purposes and are fit for use.
...
Architects, systems and software engineers, developers, and data modelers of the DGIWG and its component systems/applications in these roles will promote uniform geospatial data access, GeoPackage and GeoPackage SQLite Extension development and use. The goal is to increase interoperability, modularity, and agility of applications within the DGIWG. This interoperability standard is an adjunct to the base standard profiled – not a stand-alone document.
